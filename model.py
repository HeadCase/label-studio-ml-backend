...
class SimpleTextClassifier(LabelStudioMLBase):
    ...
    def __init__(self, **kwargs):
        ...
        # then collect all keys from config which will be used to extract data from task and to form
        # prediction Parsed label config contains only one output of <Choices> type
        ...
        self.description = self.info['inputs'][0]['value']

        # TODO: Data prepro here
    ...
    def reset_model(self):
        # Full pipeline for fittable model, initialised with fittable data
    ...
    def predict(self, tasks, **kwargs):
        ...
        input_texts = []
        for task in tasks:
            input_texts.append(task['data'][self.description])
    ...
    def fit(self, annotations, workdir=None, **kwargs):
        ...
        for completion in annotations:
            ...
            print(completion)
            if completion['annotations'][0].get('skipped') or completion[
                'annotations'
            ][0].get('was_cancelled'):
                continue

            input_text = completion['data'][self.description]
